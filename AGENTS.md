...
2) Preparar EA (el agente es quien genera/edita código MQL5):
   ...
   - Ofrecer crear un EA nuevo según reglas del usuario o editar uno existente.
     - **REGLA DE ORO DE NOMENCLATURA (SIMETRÍA DUCKDB)**: Todo parámetro de entrada en el código MQL5 (`input type variable`) **OBLIGATORIAMENTE** debe comenzar con el prefijo `Inp` (Ej: `InpBaseLot`, `InpStopLoss`). Esta convención es NO NEGOCIABLE ya que permite que `Tools-Agents/DuckDB_Analyzer.py` y el Normalizador identifiquen dinámicamente qué columnas son parámetros (inputs) y cuáles son métricas. Sin este prefijo, el análisis genético fallará al no poder separar variables de resultados.
