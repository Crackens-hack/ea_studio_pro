//+------------------------------------------------------------------+
//|  EA_AdaptiveFlow.mq5                                            |
//|  Entrada más flexible: tendencia + pullback + ATR                |
...
input ENUM_TIMEFRAMES InpTF           = PERIOD_M30;     // Marco temporal
...
input double   InpPullbackPct         = 0.15;           // Pullback mínimo vs EMA rápida (%)
input int      InpATRPeriod           = 14;             // Periodo ATR
input double   InpATR_SL_Mult         = 1.8;            // SL = ATR * mult
input double   InpATR_TP_Mult         = 2.7;            // TP = ATR * mult
...
input int      InpMagic               = 24032027;       // Magic number
...
bool CopyIndicatorBuffers(double &fast0, double &fast1, double &slow0, double &rsi0, double &atr0)
{
   double buf[];
   if(CopyBuffer(hFastEMA, 0, 0, 2, buf) < 2) return false;
   fast0 = buf[0]; fast1 = buf[1];
   if(CopyBuffer(hSlowEMA, 0, 0, 1, buf) < 1) return false;
   slow0 = buf[0];
   if(CopyBuffer(hRSI, 0, 0, 1, buf) < 1) return false;
   rsi0 = buf[0];
   ...
   for(int i=0; i<totalDeals; i++)
   {
      ...
      if(entryType == DEAL_ENTRY_IN)
      {
         ...
         else
         {
            ...
            entries[idx].time = dTime; // último fill
         }
      }
...
bool PullbackLong(double low1, double ema1)
{
   double threshold = ema1 * (1.0 - InpPullbackPct/100.0);
   return (low1 <= threshold);
}

bool PullbackShort(double high1, double ema1)
{
   double threshold = ema1 * (1.0 + InpPullbackPct/100.0);
   return (high1 >= threshold);
}
...
void OnTick()
{
   ...
   double fast0, fast1, slow0, rsi0, atr0;
   if(!CopyIndicatorBuffers(fast0, fast1, slow0, rsi0, atr0)) return;

   TrailPositions(atr0);
   if(!NewBar()) return;
   if(HasOpenPosition()) return;

   double close0 = iClose(gSymbol, InpTF, 0);
   double close1 = iClose(gSymbol, InpTF, 1);
   double high1  = iHigh(gSymbol,  InpTF, 1);
   double low1   = iLow(gSymbol,   InpTF, 1);
   double bid    = SymbolInfoDouble(gSymbol, SYMBOL_BID);
   double ask    = SymbolInfoDouble(gSymbol, SYMBOL_ASK);

   bool upTrend   = TrendUp(fast0, slow0);
   bool downTrend = TrendDown(fast0, slow0);

   bool longPull  = upTrend && PullbackLong(low1, fast1) && close0 > fast0 && rsi0 <= 60.0;
   bool shortPull = downTrend && PullbackShort(high1, fast1) && close0 < fast0 && rsi0 >= 40.0;

   if(longPull)
   {
      double sl = bid - atr0 * InpATR_SL_Mult;
      double tp = bid + atr0 * InpATR_TP_Mult;
      double stopPoints = (bid - sl) / _Point;
      double lots = CalcLotByRisk(stopPoints);
      if(lots > 0)
         trade.Buy(lots, gSymbol, ask, sl, tp, "Pullback long");
   }
   else if(shortPull)
   {
      double sl = ask + atr0 * InpATR_SL_Mult;
      double tp = ask - atr0 * InpATR_TP_Mult;
      double stopPoints = (sl - ask) / _Point;
      double lots = CalcLotByRisk(stopPoints);
      if(lots > 0)
         trade.Sell(lots, gSymbol, bid, sl, tp, "Pullback short");
   }
}
...
double OnTester()
{
   ...
   PrintFormat("OnTester -> PF: %.2f | MaxDD: %.2f%% | Recov: %.2f | Winrate: %.2f%% | Trades: %.0f | Payoff: %.2f | PR: %.2f | Sharpe: %.2f | AvgHold: %.1f min | Fitness: %.4f",
